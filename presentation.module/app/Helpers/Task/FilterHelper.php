...
namespace PresentModule\App\Helpers\Task;
...
class FilterHelper extends Common
{
		...
		foreach ($filters as $fieldId => $filterRow)
		{
			...
			switch ($filterRow['type'])
			{
				...
				case 'entity_selector':
					$data = $this->getEntitySelectorFilterFieldData($filterRow);
					if ($data)
					{
						$ufFilter = array_merge($ufFilter, $data);
					}
	...
	private function getFilterRaw()
	{
		...
		if ($this->getGroupId() && in_array('STAGE_ID', $fields))
		{
			$stageItems = [];
			$stages = \Laravel\Illuminate\App\Models\B24\Task\TaskStage::where('ENTITY_TYPE', '=', 'G')
				->where('ENTITY_ID', '=', $this->getGroupId())->get();

			foreach ($stages as $stage) {
				if ($stage->SYSTEM_TYPE == 'NEW') {
					$stageItems['0,'.$stage->ID] = $stage->TITLE;
				} else {
					$stageItems[$stage->ID] = $stage->TITLE;
				}
			}

			$filter['STAGE_ID'] = array(
				'id' => 'STAGE_ID',
				'name' => 'Стадия',
				'type' => 'list',
				'params' => array(
		...
		if (!empty($uf))
		{
			foreach ($uf as $item)
			{
				...
				$available = ['datetime', 'string', 'double', 'boolean', 'crm', /*'hlblock', 'hlblock_detail'*/];
		...
		$arItems = [];
		$objects = \PresentModule\App\Models\Object\ConstructionObject::select(['ID', 'UF_ADDRESS', 'UF_UID'])->get();
		foreach ($objects as $object) {
			$uid = str_pad($object->UF_UID, 3, '0', STR_PAD_LEFT);
			$arItems[] = [
				'id' => $object->ID,
				'entityId' => 'my-entity',
				'title' => $object->UF_ADDRESS.', №'.$uid,
				'tabs' => 'my-tab'
			];
		}
